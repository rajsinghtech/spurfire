...
**Spurfire** is a third-person, peer-hosted open-range movement shooter where every player fights from horseback, performs dangerous flying dismounts, builds Spur with stylish riding, and battles across terrain that scales with the size of the lobby. Today the prototype includes mounted locomotion, the SF-series rifle range, and M2 Saddle Dive. Saddle Dive is **playtest pending, not done**. M3, M4, and M5 are source-complete and playtest pending: their native authority runs through signed wire-2.0 replication and complete-state migration, Godot presents the horse-loss/return arc, Majestic Charge, and Bounty Run world/results loop, and secret-free telemetry measures the remaining human acceptance gates. M6's peer-owned election, 150 ms rewind cap, native secret handoff, signed 20 Hz actor deltas with recoverable 2 Hz bases, hash-bound MatchState fragmentation, bounded input replay, ten-second presentation ring, privacy-suppressed public stats, and credential-free 6/8/12/16-player soak proof are implemented; credentialed 8–16-player qualification remains (see `docs/prototype-plan.md`).
...
- `just scale-proof` — credential-free signed 6/8/12/16-player virtual scale and soak gate.
...
The control plane, protocol, CLI, and HTTP lobby prototype implement organization tailnet-per-lobby provisioning with an encrypted exact-tuple file vault, writer fencing, and mutation-closed startup reconciliation. Organization child create/token/delete is live-proven; child-scoped one-use enrollment is implemented and mock-tested but still needs live end-to-end verification. The main control plane never joins a lobby tailnet. Public real activation remains closed until workload identity/external custody, persistent abuse controls, restrictive child-policy proof, orphan-remediation exercises, and the remaining operations gates are complete; the hosted preview remains dry-run. Godot 4.7.1 plus Rust GDExtension provides mounted movement/combat (M0–M1), the M2 Saddle Dive implementation, and a native `PeerSession`. M2 is **implementation complete / playtest pending**: deterministic and headless gates do not replace the natural frequency, hit-rate, post-landing-death, notification, and animation checks in `docs/prototype-plan.md`. The invited-friends lobby selects signed wire 2.0 and routes M3/M4 actor, horse, Spur, and Charge state through the composed native authority and atomic migration. Remote proxies present spook, bolt, Majestic Return, and archetype Charge motion. The checkpointed no-decay economy validates all fill rows, contextual Q spends, six-second modifiers, readiness HUD/audio, and secret-free per-actor acceptance telemetry. M3 and M4 are therefore **source complete / playtest pending**, not accepted by automated checks alone. M5 has a checkpoint-ready deterministic match clock, score ledger, assist/respawn timing, Most Wanted schedule, and objective cycle. Combat score producers commit transactionally with combat, and the complete match state migrates under the same signed checkpoint. Authority-only two-hertz MatchState keyframes use one MTU-safe packet through eight players and hash-bound independently signed fragments for larger rosters; no partial score truth is exposed. The game resizes the active flat territory from the roster-scaled radius, derives edge-buffered objective and separated outer-ring respawn points from the lobby seed, applies full-state respawns and temporary buffs, and presents/executes all five objective types plus the Most Wanted world flare. Ammo-wagon refill and match scoring commit atomically. The results overlay ranks the final ledger, shows category contributions, records the play-again answer, and lets the authority submit bounded results through the native capability boundary before safe teardown. Secret-free M5 intervals and the deterministic aggregator measure score pacing/category diversity, reveal pressure, respawn/encounter/objective time, worst convergence gap, and play-again rate. M5 is therefore **source complete / playtest pending**. M6 now has one peer-owned election rule from the immutable match-start matrix, service-validated successor heartbeats, exact migrated election-hash propagation, the complete M3–M5 checkpoint, a 150 ms rewind admission cap, signed 20 Hz actor deltas against recoverable 2 Hz bases, bounded local input replay after reconciliation, a 600-snapshot presentation ring, native zeroizing create/join HTTPS, and a privacy-suppressed public stats endpoint. A credential-free three-process wire-2 proof kills the original authority and verifies exact M5 score, clock, and objective continuity after fragmented checkpoint installation. The credential-free scale gate runs full virtual 15-minute 6/8/12/16-player sessions with modeled loss/relay, churn, failover, MTU checks, and bot-duel fairness. Credentialed scale/churn/soak evidence, human acceptance, and RustScale's remaining platform/telemetry gaps still block Alpha; see `docs/prototype-plan.md`, `docs/decisions.md`, and `docs/control-plane-network-view.md`.
