...
**Spurfire** is a third-person, peer-hosted open-range movement shooter where every player fights from horseback, performs dangerous flying dismounts, builds Spur with stylish riding, and battles across terrain that scales with the size of the lobby. Today the prototype includes mounted locomotion, the SF-series rifle range, M2 Saddle Dive, M3 horse-loss/return, M4 Majestic Charge, and the M5 Bounty Run loop. Those systems are source-complete and entering invited Alpha playtesting: the native authority runs through signed wire-2.0 replication and complete-state migration, while secret-free telemetry measures the human feel and balance questions that Alpha exists to answer. M6's peer-owned election, rewind, native secret handoff, signed replication, bounded replay, presentation history, privacy-suppressed stats, and credential-free scale proof are implemented; further credentialed scale findings are Alpha work (see `docs/prototype-plan.md`).
...
docs/release-notes-0.2.0.md  Alpha scope, verification, and in-Alpha playtest targets
...
Every pull request runs `just check` on Ubuntu and macOS plus the bounded Godot 4.7.1 smoke suite on Linux. The nonpublishing **Client Preflight** workflow builds Linux x86_64, Linux ARM64, and macOS universal archives for invited Alpha testing. It uploads only short-lived workflow artifacts; it does not create a tag or public release, publish a package, advance an alias, or require platform-signing credentials. Windows is outside the Alpha platform set. See [docs/testing.md](docs/testing.md) and [docs/deployment.md](docs/deployment.md).
...
The control plane, protocol, CLI, and HTTP lobby prototype implement organization tailnet-per-lobby provisioning with an encrypted exact-tuple file vault, writer fencing, and mutation-closed startup reconciliation. Organization child create/token/delete and direct one-use client enrollment are live-proven; the integrated restrictive-policy and custody path still needs its separately authorized hosted exercise. The main control plane never joins a lobby tailnet. Public real activation remains closed until workload identity/external custody, persistent abuse controls, restrictive child-policy proof, orphan-remediation exercises, and the remaining operations gates are complete; the hosted preview remains dry-run. Godot 4.7.1 plus Rust GDExtension provides the M0–M5 gameplay loop and native `PeerSession`. Deterministic, headless, process, and live-scale gates protect the build but do not answer the natural frequency, hit-rate, post-landing danger, notification, animation, pacing, comfort, and play-again questions in `docs/prototype-plan.md`; those are explicitly measured during Alpha. Hosted real-mode activation remains separately gated, but human gameplay testing of the checksummed invited Alpha build is not blocked on public operations, Windows, signing, or publication.
