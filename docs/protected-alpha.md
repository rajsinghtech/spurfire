...
The named Kubernetes `coordination.k8s.io/v1` Lease is the external anti-rollback CAS authority. Its annotation repeats installation ID, the receipt's pre-install resourceVersion, store ID, receipt digest, lobby/generation, supervisor epoch, state hash, phase, and both deadlines. Installing that annotation advances the live Kubernetes resourceVersion; the preserved pre-install version binds the receipt while the new live version starts the broker's optimistic-CAS chain. Every mutating broker frame checks the exact UID/current resourceVersion/binding and performs a Kubernetes optimistic `PUT` CAS. A PVC copy or rollback has no resourceVersion authority and cannot reopen admission. Ambiguous tuples quarantine; `cleanup_only`, `released`, and `quarantined` phases cannot admit.
