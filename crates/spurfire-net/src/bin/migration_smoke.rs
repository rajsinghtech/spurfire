...
async fn child_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let start = raw_key.iter().position(|byte| !byte.is_ascii_whitespace()).unwrap_or(raw_key.len());
    let end = raw_key.iter().rposition(|byte| !byte.is_ascii_whitespace()).map_or(start, |index| index + 1);
